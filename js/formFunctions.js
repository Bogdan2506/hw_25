...
function validateUser(user) {
    ...
    if (!name.match(nameRegex)) {
        return "Error name. Please enter a correct name.";
    }

    if (!email.match(emailRegex)) {
        return "Error email address. Please enter a valid email address.";
    }

    if (!age.toString().match(ageRegex)) {
        return "Error age. Please enter a valid age.";
    }

    if (!phone.match(phoneRegex)) {
        return "Error phone number. Please enter a valid 11-digit phone number.";
    }

    if (!card.match(cardRegex)) {
        return "Error card number. Please enter a valid 16-digit card number.";
    }
    return isValid;
}
